...
def create_data(arrays, inputvars, Ntrk, Nsv):
...
best_model_check =  ModelCheckpoint("GNN_best_Disp_noEFrac.h5", monitor = "val_loss", verbose=1, save_best_only=True, save_weights_only=False)
...
model=Model(name='model_CNN', inputs = [input_dijet, input_SV, input_trks, input_trksvs], outputs = output)
...
history = model.fit(x = [train_dijet, train_svs, train_trks, train_trktosv], y = train_label, sample_weight = train_weights, epochs=150, batch_size=500, verbose=1, validation_data = (([test_dijet, test_svs, test_trks, test_trktosv], test_label, test_weights)), callbacks=[best_model_check])

with open('GNN_history_final_Disp_noEFrac.json', 'w') as f:
    json.dump(history.history, f)



model.save("GNN_full_final_Disp_noEFrac.h5")
