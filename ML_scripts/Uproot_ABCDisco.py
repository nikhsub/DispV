...
def custom_loss(y_true, y_pred):
    ...
    return cross_entropy+coeff*corr_eff
...
#
sig_root_files = glob.glob("{0}/ggHbbbb_M40*_Pt40_NewNew/*.root".format(sig_input_path))
...
train_dijet, test_dijet, train_svs, test_svs, train_trks, test_trks, train_trktosv, test_trktosv, train_jet1, test_jet1, train_jet2, test_jet2, train_weights, test_weights, train_label, test_label = train_test_split(total_data["dijet_vars"], total_data["SV_vars"], total_data["trk_vars"], total_data["trkTosv_vars"], total_data["jet1ptrk_vars"], total_data["jet2ptrk_vars"], total_data["weights"], total_data["isSignal"], test_size=0.2, shuffle=True) 
...
best_model_check =  ModelCheckpoint("ABCD_best_Pt40_complete_corr_"+args.output+".h5", monitor = "val_loss", verbose=1, save_best_only=True, save_weights_only=False)
...
cov_sv, cov_trks = GNN_block_shallow(input_SV, input_trks, input_trksvs, ntrk=Ntrk, nsv=Nsv)
...
model=Model(name='model_GNN', inputs = [input_jet1, input_jet2, input_dijet, input_SV, input_trks, input_trksvs], outputs = output)

#model.compile(optimizer = optimizer, loss = 'binary_crossentropy', metrics=['accuracy', roc_auc])
model.compile(optimizer = optimizer, loss = custom_loss, metrics=['accuracy', roc_auc, 'BinaryCrossentropy', bkg_corr])
model.summary()
...
history = model.fit(x = [train_jet1, train_jet2, train_dijet, train_svs, train_trks, train_trktosv], y = train_label, sample_weight = train_weights, epochs=args.epochs, batch_size=500, verbose=1, validation_data = (([test_jet1, test_jet2, test_dijet, test_svs, test_trks, test_trktosv], test_label, test_weights)), callbacks=[best_model_check])


with open('ABCD_history_final_Pt40_complete_corr_'+args.output+'.json', 'w') as f:
    json.dump(history.history, f)



model.save("ABCD_full_final_Pt40_complete_corr_"+args.output+".h5")
