...
import CNNblock
from CNNblock import CNN_block, GNN_block, GNN_block_shallow, GNN_ptrk_block, CNN_ptrk_block
...
inputvars["ptrk_vars"] = ['ptrkIP2D', 'ptrkIP2Dsig', 'ptrkIP3Dminsig', 'ptrkEFrac', 'ptrkjet1', 'ptrkjet2']
...
bak170_root_files = glob.glob("{0}/QCD_postEE_Pt170to300_Trigger_newPtrk_full_officialMC_DispTrained_bestDNNd/*.root".format(bak_input_path))
bak300_root_files = glob.glob("{0}/QCD_postEE_Pt300to470_Trigger_newPtrk_full_officialMC_DispTrained_bestDNNd/*.root".format(bak_input_path))
...
bak170_data = create_data(bak170_arrays, inputvars, Ntrk ,Nsv, Nptrk)
bak300_data = create_data(bak300_arrays, inputvars, Ntrk, Nsv, Nptrk)
...
for key in sig_data.keys():
    ...
    #total_data[key] = np.concatenate((sig_data[key],  bak300_data[key]))
...
perepoch_model_check =  ModelCheckpoint("models/PromptOnlyNewPtrk_CNN_iter{epoch:02d}_Pt40_complete_decorr_"+args.output+".h5", monitor = "val_loss", verbose=1, save_best_only=False, save_weights_only=False, mode='auto', period=1)
best_model_check =  ModelCheckpoint("models/PromptOnlyNewPtrk_CNN_best_Pt40_complete_decorr_"+args.output+".h5", monitor = "val_loss", verbose=1, save_best_only=True, save_weights_only=False)
...
cov_ptrk = CNN_ptrk_block(input_ptrk)
...
with open('PromptOnly_CNN_history_final_Pt40_complete_decorr_'+args.output+'.json', 'w') as f:
    json.dump(history.history, f)



model.save("models/PromptOnly_CNN_full_final_Pt40_complete_decorr_"+args.output+".h5")
