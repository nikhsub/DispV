...
import CNNblock
from CNNblock import CNN_block, GNN_block, GNN_block_shallow
...
def custom_loss(y_true, y_pred):
    ...
    corr_eff_square = corr(bkg_pred[:, 0]*bkg_pred[:, 0], bkg_pred[:, 1]*bkg_pred[:, 1]) 
    abcd_ratio = ABCD_ratio(bkg_pred[:, 0], bkg_pred[:, 1]) 
    bce = tf.keras.losses.BinaryCrossentropy()
    cross_entropy = bce(y_true, y_pred)
    return cross_entropy+coeff*(corr_eff+corr_eff_square)#+0.01*abcd_ratio
...
variables = inputvars["dijet_vars"]+inputvars["SV_vars"]+inputvars["trk_vars"]+inputvars["trkTosv_vars"]#+inputvars["jet1ptrk_vars"]+inputvars["jet2ptrk_vars"]
...
sig_root_files_M40 = glob.glob("{0}/ggHbbbb_*M40_*mm_*/*.root".format(sig_input_path))
sig_root_files_M55 = glob.glob("{0}/ggHbbbb_*M55_*mm_*/*.root".format(sig_input_path))
sig_root_files_M30 = glob.glob("{0}/ggHbbbb_*M30_*mm_*/*.root".format(sig_input_path))
sig_root_files = sig_root_files_M30+sig_root_files_M40+sig_root_files_M55

bak170_root_files = glob.glob("{0}/QCD_Pt170to300_Trigger_newPtrk_qcd_full/*.root".format(bak_input_path))
bak300_root_files = glob.glob("{0}/QCD_Pt300to470_Trigger_newPtrk_qcd_full/*.root".format(bak_input_path))

bak170_weight = 11798/12634423
bak300_weight = 7823/55842943
...
bak170_arrays = read_files(bak170_root_files, variables, show_progressbar=True)
bak300_arrays = read_files(bak300_root_files, variables, show_progressbar=True)
...
n_bak170 = len(bak170_arrays['dijetnSV'])
n_bak300 = len(bak300_arrays['dijetnSV'])
n_total = n_bak170+n_bak300
weight170 = 1/(float(n_total))*(bak170_weight/(bak170_weight+bak300_weight))
weight300 = 1/(float(n_total))*(bak300_weight/(bak170_weight+bak300_weight))

label_bak170 = np.zeros(n_bak170).astype(int)
weight_bak170 = np.ones(n_bak170)*weight170
bak170_arrays["isSignal"] = label_bak170
bak170_arrays["weights"] = 100*weight_bak170

label_bak300 = np.zeros(n_bak300).astype(int)
weight_bak300 = np.ones(n_bak300)*weight300
bak300_arrays["isSignal"] = label_bak300
bak300_arrays["weights"] = 100*weight_bak300
...
#print(bak170_arrays)
#print(sig_arrays)
sig_data = create_data(sig_arrays, inputvars, Ntrk, Nsv)
bak170_data = create_data(bak170_arrays, inputvars, Ntrk ,Nsv)
bak300_data = create_data(bak300_arrays, inputvars, Ntrk, Nsv)
...
print(bak170_data["isSignal"].shape)
print(bak300_data["isSignal"].shape)
#print(bak_data)

total_data = {}

for key in sig_data.keys():
    total_data[key] = np.concatenate((sig_data[key], bak170_data[key], bak300_data[key]))
...
train_dijet, test_dijet, train_svs, test_svs, train_trks, test_trks, train_trktosv, test_trktosv, train_weights, test_weights, train_label, test_label = train_test_split(total_data["dijet_vars"], total_data["SV_vars"], total_data["trk_vars"], total_data["trkTosv_vars"], total_data["weights"], total_data["isSignal"], test_size=0.2, shuffle=True) 
...
perepoch_model_check =  ModelCheckpoint("models/DispOnly_officialMC_iter{epoch:02d}_Pt40_complete_corr_"+args.output+".h5", monitor = "val_loss", verbose=1, save_best_only=False, save_weights_only=False, mode='auto', period=1)
best_model_check =  ModelCheckpoint("models/DispOnly_officialMC_best_Pt40_complete_corr_"+args.output+".h5", monitor = "val_loss", verbose=1, save_best_only=True, save_weights_only=False)
...
#lstm_jet1   = LSTM(name="lstm_jet1", units =35, go_backwards=True, implementation=2)(cov_jet1)
#lstm_jet2   = LSTM(name="lstm_jet2", units =35, go_backwards=True, implementation=2)(cov_jet2)
...
#output = Concatenate()([output_prompt, output_disp])


model=Model(name='model_GNN', inputs = [input_dijet, input_SV, input_trks, input_trksvs], outputs = output_disp)

model.compile(optimizer = optimizer, loss = 'binary_crossentropy', metrics=['accuracy', roc_auc, 'binary_crossentropy'])
...
history = model.fit(x = [train_dijet, train_svs, train_trks, train_trktosv], y = train_label, sample_weight = train_weights, epochs=args.epochs, batch_size=500, verbose=1, validation_data = (([test_dijet, test_svs, test_trks, test_trktosv], test_label, test_weights)), callbacks=[perepoch_model_check, best_model_check, earlystop])


with open('DispOnly_officialMC_history_final_Pt40_complete_corr_'+args.output+'.json', 'w') as f:
    json.dump(history.history, f)



model.save("models/DispOnly_officialMC_full_final_Pt40_complete_corr_"+args.output+".h5")
