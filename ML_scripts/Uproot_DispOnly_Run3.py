...
sig_root_files_M40 = glob.glob("{0}/ggHbbbb_M40_*mm_private_trigger/*.root".format(sig_input_path))
sig_root_files_M55 = glob.glob("{0}/ggHbbbb_M55_*mm_private_trigger/*.root".format(sig_input_path))
...
bak_data = create_data(bak_arrays, inputvars, Ntrk ,Nsv)
...
perepoch_model_check =  ModelCheckpoint("models/DispOnly_iter{epoch:02d}_Pt40_complete_corr_"+args.output+".h5", monitor = "val_loss", verbose=1, save_best_only=False, save_weights_only=False, mode='auto', period=1)
best_model_check =  ModelCheckpoint("models/DispOnly_best_Pt40_complete_corr_"+args.output+".h5", monitor = "val_loss", verbose=1, save_best_only=True, save_weights_only=False)
...
with open('DispOnly_history_final_Pt40_complete_corr_'+args.output+'.json', 'w') as f:
    json.dump(history.history, f)



model.save("models/DispOnly_full_final_Pt40_complete_corr_"+args.output+".h5")
