// -*- C++ -*-
//
// Package:    DisplacedDJ_MiniAOD/DijetIVFMiniMLDisp_NewPtrk_dispprompttensor
// Class:      DijetIVFMiniMLDisp_NewPtrk_dispprompttensor
// 
/**\class DijetIVFMiniMLDisp_NewPtrk_dispprompttensor DijetIVFMiniMLDisp_NewPtrk_dispprompttensor.cc DisplacedDJ_MiniAOD/DijetIVFMiniMLDisp_NewPtrk_dispprompttensor/plugins/DijetIVFMiniMLDisp_NewPtrk_dispprompttensor.cc
...
#include "DisplacedDJ_MiniAOD/DJAnalyzerMini/interface/DijetIVFMiniMLDisp_NewPtrk_dispprompttensor.h"
...
//
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::DijetIVFMiniMLDisp_NewPtrk_dispprompttensor(const edm::ParameterSet& iConfig)
:
  ...
  slimntuple_ (iConfig.getUntrackedParameter<bool>("slim")),
...
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::~DijetIVFMiniMLDisp_NewPtrk_dispprompttensor()
{
...
void
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   using namespace pat;
  ...
  //initialize the tensors for ML
   ...
   std::vector<std::string> input_prompt_names_{"input_ptrk:0",  "input_SVDNNd:0"};
   ...
   //tensorflow::Tensor input_prompt(tensorflow::DT_FLOAT, {1, 20, 7});
   ...
   for(size_t i=0; i<calojet_coll->size()-1; i++){
      for(size_t k=i+1; k<calojet_coll->size(); k++){
             ...
             // }
              ...
              //input_prompt.flat<float>().setZero();
              ...
              for(size_t j = 0; j<dijettrks2_clean.size(); j++){
               ...
               std::map<int, float> jet1PVptsum;
               std::map<int, float> jet2PVptsum;
               ...
               for(size_t j = 0; j<dijettrks_clean_ordered.size(); j++){
                   ...
                   trksPt+=trk->pt();
                   if(indices_ordered[j]==1 && ip3d_minsig<4.0){
                       if(jet1PVptsum.find(pvidx)==jet1PVptsum.end()){
                           jet1PVptsum[pvidx] = trk_energy;
                       } 
                       else{
                           jet1PVptsum[pvidx]+=trk_energy;
                       }
                   ...
                   if(indices_ordered[j]==2 && ip3d_minsig<4.0){
                       if(jet2PVptsum.find(pvidx)==jet2PVptsum.end()){
                           jet2PVptsum[pvidx] = trk_energy;
                       }
                       else{
                           jet2PVptsum[pvidx]+=trk_energy;
                       }
              ...
              for (const auto &p: jet1PVptsum){
                  if(p.second>jet1maxpt) jet1maxpt=p.second;
              } 
   
              for (const auto &p: jet2PVptsum){
                  if(p.second>jet2maxpt) jet2maxpt=p.second;
              }
         ...
         ++cluster) {
              ...
              //// if( n_tks_jet1 >=1 && n_tks_jet2 >=1 ) dijet_tag = true;
               ...
               std::vector<int> tmp_pvidx = pvidx_prompt;
               ...
               std::reverse(tmp_pvidx.begin(), tmp_pvidx.end());
               ...
               for(size_t iptrk = 0; iptrk<nptrk; iptrk++){
                   if(iptrk<20){
                       //float* ptr1 = &(input_prompt).tensor<float, 3>()(0, iptrk, 0);
                       float* ptr1 = &(_input_prompt_tensors.at(0).second).tensor<float, 3>()(0, iptrk, 0);
                       *ptr1 = nanTozero(float(tmp_pvidx.at(iptrk))); 
                       ...
                       *(++ptr1) = nanTozero(float(std::tanh(tmp_ip3dminisig.at(iptrk)/10)-1));
                       //*(++ptr1) = nanTozero(float(std::tanh(tmp_energys.at(iptrk)/(jet1_ref->pt()*jetcorrfac.at(i)+jet2_ref->pt()*jetcorrfac.at(k)))));
                       //*(++ptr1) = nanTozero(float(std::tanh(tmp_ip3dminisig.at(iptrk)/10)-1));
                       if(tmp_indices.at(iptrk)==1){
                           *(++ptr1)=1.0;
                           *(++ptr1)=0.0;
                       }
                       else{
               ...
               *(++ptr) = nanTozero(float(nvtx));
               ...
               //tensorflow::run(_Promptsession, {{"input_ptrk:0", input_prompt}}, {"Identity:0"}, &promptoutputs);
               ...
               //SVdphiLxyVX.push_back(dphiLxyVX);
                ...
                SVjet1maxPVPt.push_back(jet1maxpt);
                SVjet2maxPVPt.push_back(jet2maxpt);
...
void 
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::beginJob()
{
   ...
   tree->Branch("SVjet1maxPVPt", &SVjet1maxPVPt);
   tree->Branch("SVjet2maxPVPt", &SVjet2maxPVPt);
...
void 
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::endJob() 
{
}
...
// 
void
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::beginLuminosityBlock(edm::LuminosityBlock const& iLumi, edm::EventSetup const& iSetup)
{  
...
void 
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::endLuminosityBlock(edm::LuminosityBlock const& iLumi, edm::EventSetup const& iSetup)
{}
...
void
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void  
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::jettracks (JetTrks* fAssociation, const edm::Handle<edm::View<reco::Jet> >&  jetcoll, const std::vector<reco::Track>& tracks, float cone){
...
void 
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::vertexMerge(std::vector<TransientVertex> &VTXs, double maxFraction, double minSignificance){
    VertexDistance3D dist;
...
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::TrackVertexRefit(std::vector<reco::TransientTrack> &Tracks, std::vector<TransientVertex> &VTXs){
...
bool 
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::isGoodVtx(TransientVertex& tVTX){
...
std::vector<size_t> 
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::OrderTracks(const reco::TrackRefVector&  inputTracks, const reco::Vertex& pv, const TransientTrackBuilder& theB){
...
std::vector<size_t>
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::OrderSVs(const std::vector<TransientVertex> & inputvtx, const reco::Vertex& pv){
...
void
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::process(std::string line, char delim, std::vector<std::string>& fields){
...
float 
DijetIVFMiniMLDisp_NewPtrk_dispprompttensor::nanTozero(float input){
...
DEFINE_FWK_MODULE(DijetIVFMiniMLDisp_NewPtrk_dispprompttensor);
